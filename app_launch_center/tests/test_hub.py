...
def test_hub_home_and_status(hub_client):
    ...
    assert "One on One Meeting" in apps.text
    assert "Meeting 41 B" in apps.text
    ...
    for app in status["apps"]:
        ...
        assert ":4720" in app["url"] or app["url"].startswith("http://127.0.0.1:4720")


def test_launch_apps_stay_on_4720(hub_client):
    client, _ = hub_client
    for app_id, path in [
        ("meeting_41b", "/meeting-41b.html"),
        ("one_on_one", "/meeting-simulation.html"),
        ...
        assert res["url"].endswith(path) or res["url"].rstrip("/").endswith(path.rstrip("/") or "")
        assert "4720" in res["url"]


def test_meeting_41b_available_on_same_app(hub_client):
    client, _ = hub_client
    ...
    ensured = client.post("/api/meetings/41b/ensure").json()
    assert ensured["meeting"]["status"] == "live"
    mid = ensured["meeting"]["id"]
    room = client.get(f"/meeting/{mid}")
